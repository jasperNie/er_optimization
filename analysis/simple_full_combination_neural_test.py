#!/usr/bin/env python3
"""
Simple Full Combination Neural Test
...
def evaluate_full_combination(num_nurses=4):
    ...
    print("\n" + "=" * 80)
    print(f"   FULL COMBINATION NEURAL TEST - TRAIN ALL, TEST RANDOM MIX")
    print("=" * 80)
    ...
    training_log_path = f"logs/full_combination_test/neural_training_{num_nurses}nurses.txt"
    os.makedirs(os.path.dirname(training_log_path), exist_ok=True)
    neural_policy = optimizer.run(training_log_path)
    print("Training complete!")
    
    # Create callable function from neural policy (EXACT copy from working script)
    ...
    if esi_weighted_filtered and mts_weighted_filtered and weighted_waits:
        ...
        print(f"   Neural Network: {neural_avg_weighted_hours:.2f} hours weighted ({neural_avg_wait_hours:.2f} hours avg)")
    ...
    analysis_log_path = f"logs/full_combination_test/neural_full_combination_analysis_{num_nurses}nurses.txt"
    os.makedirs(os.path.dirname(analysis_log_path), exist_ok=True)
    
    with open(analysis_log_path, "w", encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write(f"   FULL COMBINATION NEURAL TEST RESULTS - {num_nurses} NURSES\n")
        f.write("=" * 80 + "\n\n")
